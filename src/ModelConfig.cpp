...
// ── Gains parser ─────────────────────────────────────────────────────────────
//
// Expected format (per-model gains.yaml, generated by colosseum export_gains):
//   Left_Hip_Pitch.kp: 33.09
//   Left_Hip_Pitch.kd: 3.95
//   Left_Hip_Pitch.default_pos: -0.24
//   ...
//
GainsMap ModelConfig::parse_gains(const fs::path& yaml_path) {
    GainsMap gains;
    std::string content = read_file(yaml_path);
    std::istringstream in(content);
    std::string line;
    ...
    while (std::getline(in, line)) {
        ...
        auto [key, value] = split_kv(line);  // key = "Joint.field"
        if (key.empty() || value.empty()) continue;

        auto dot = key.rfind('.');
        if (dot == std::string::npos) continue;
        std::string joint = key.substr(0, dot);
        std::string field = key.substr(dot + 1);
        float v = std::stof(value);

        JointGains& g = gains[joint];
        if      (field == "kp")           g.kp           = v;
        else if (field == "kd")           g.kd           = v;
        else if (field == "armature")     g.armature     = v;
        else if (field == "frictionloss") g.frictionloss = v;
        else if (field == "effort_limit") g.effort_limit = v;
        else if (field == "default_pos")  g.default_pos  = v;
        else if (field == "action_scale") g.action_scale = v;
    }

    if (gains.empty())
        throw std::runtime_error(yaml_path.string() + ": no gains parsed");
